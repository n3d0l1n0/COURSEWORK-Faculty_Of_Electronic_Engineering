...
void CRacunarskaGrafikaLV1GDIPripremaView::DrawShapes(CDC* pDC)
{
    ...
    // veliki pravougaonik
    {
        ...
        oldPen = pDC->SelectObject(&outline);
        oldBrush = pDC->SelectObject(&emptyBrush); 
    ...
    int left = 1 * cellSize;
    int top = 3 * cellSize;
    int right = 500 - 1 * cellSize;
    int bottom = 500 - 3 * cellSize;
...
void CRacunarskaGrafikaLV1GDIPripremaView::DrawRegularPolygon(CDC* pDC, int cx, int cy, int r, int n, float rotAngle)
{
	...
	for (int i = 0; i < n; i++)
	{
		float a = rot + i * (2.0f * 3.1415926535f / n);
		float x = cx + r * cos(a);
		float y = cy + r * sin(a);
...
{
	CPen pen(PS_SOLID, 1, RGB(180, 255, 255));
